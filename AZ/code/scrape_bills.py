...
class AZBillScraper():
    def scrape_bill(self, uuid, state, state_bill_id, session):
        bill_history_data, last_status, action_ids = self.scrape_bill_history(uuid, state, state_bill_id, session)
        self.write_file(uuid, "bill_history", bill_history_data)

        bill_meta_data, internal_id = self.scrape_bill_metadata(uuid, state, state_bill_id, session, last_status)
        self.write_file(uuid, "bill_metadata", bill_meta_data)

        sponsors_data = self.scrape_sponsors(uuid, state, state_bill_id, session, internal_id)
        self.write_file(uuid, "sponsors", sponsors_data)

        for voting in self.scrape_votes(uuid, state, state_bill_id, session, internal_id, action_ids):
            voting_data, date = voting

            file_name = "{}_{}".format(uuid, date)
            self.write_file(file_name, "votes", voting_data)

    def scrape_bill_metadata(self, uuid, state, state_bill_id, session, last_status):
    ...
    def scrape_sponsors(self, uuid, state, state_bill_id, session, internal_id):
    ...
    def scrape_bill_history(self, uuid, state, state_bill_id, session):
        session_id = self.get_session_id(session)
        bill_json_url = (
    ...
    def scrape_votes(self, uuid, state, state_bill_id, session, internal_id, action_ids):
        response_key = {
            "Y": "Yea",
            "N": "Nay",
            "AB": "Absent",
...
if __name__ == "__main__":
    ...
    test.scrape_bill(test_uuid, test_state, test_bill_id, test_session)
